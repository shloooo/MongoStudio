...
import CollectionTabBar from './components/CollectionTabBar.jsx';
    ...
    setContextMenu({
      ...
      items: [
        ...
        { separator: true },
        {
          ...
          onClick: async () => {
            const res = await window.api.data.exportCollection({ connId, dbName, collection, format: 'json' });
            if (res.ok) setStatus({ type: 'info', message: `Exported ${res.count} document(s) to ${res.filePath}` });
          }
        },
        {
          label: 'Export as CSV...',
    ...
    <div className="app-root">
      ...
      <div className="app-shell">
        ...
        <main className="main-area">
          {status && <div className={`status-bar ${status.type === 'error' ? 'is-error' : 'is-info'}`}>{status.message}</div>}
          <CollectionTabBar
            tabs={tabs}
          ...
          {tabs.length === 0 ? (
            ...
            tabs.map((tab) => (
              <div key={tab.id} style={{ display: tab.id === activeTabId ? 'flex' : 'none', flex: 1, minWidth: 0, overflow: 'hidden' }}>
                <CollectionView
                  selection={tab}
        ...
        {showSettings && (
          <SettingsPage connections={connections} onClose={() => setShowSettings(false)} onImported={refreshConnections} />
        )}
        {contextMenu && (
          <ContextMenu
            x={contextMenu.x}
