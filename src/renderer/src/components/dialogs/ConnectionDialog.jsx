            ...
            <div className="modal" onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mode-toggle">
                        <label>
                            <input type="radio" checked={form.mode === 'basic'}
                                   onChange={() => update('mode', 'basic')}/>
                    ...
                    {form.mode === 'uri' ? (
                        ...
                        <>
                            ...
                            <label className="section-toggle">
                                <input type="checkbox" checked={form.useAuth}
                            ...
                            <div className="row checkboxes">
                                <label><input type="checkbox" checked={form.srv}
                                              onChange={(e) => update('srv', e.target.checked)}/> {t('dialogs.connection.srv')}
                                </label>
                                <label><input type="checkbox" checked={form.tls}
                                              onChange={(e) => update('tls', e.target.checked)}/> {t('dialogs.connection.tls')}
                                </label>
                            </div>
                        </>
                    )}
                    ...
                    <label className="section-toggle">
                        <input type="checkbox" checked={form.useSsh}
                        ...
                        <div className="ssh-section">
                            <div className="row">
                                <div>
                                    ...
                                    <input value={form.ssh.host} onChange={(e) => updateSsh('host', e.target.value)}
                                           placeholder="bastion.example.com"/>
                                </div>
                                <div>
                                    <label>{t('dialogs.connection.sshPort')}</label>
                                    <input type="number" value={form.ssh.port}
                            ...
                            <div className="mode-toggle">
                                <label>
                                    <input type="radio" checked={form.ssh.authType === 'password'}
                                           onChange={() => updateSsh('authType', 'password')}/>
                                    {t('dialogs.connection.sshAuthPassword')}
                                </label>
                                <label>
                                    <input type="radio" checked={form.ssh.authType === 'key'}
                                           onChange={() => updateSsh('authType', 'key')}/>
                            ...
                            {form.ssh.authType === 'password' ? (
                                <>
                                    ...
                                    <input type="password" value={form.ssh.password}
                                ...
                                <>
                                    ...
                                    <div className="file-picker-row">
                                        <input value={form.ssh.privateKeyPath}
                                               onChange={(e) => updateSsh('privateKeyPath', e.target.value)}
                                               placeholder="~/.ssh/id_ed25519"/>
                                        <button type="button"
                                                onClick={handlePickKey}>{t('dialogs.connection.browse')}</button>
                                    </div>
                                    <label>{t('dialogs.connection.passphrase')}</label>
                                    <input type="password" value={form.ssh.passphrase}
