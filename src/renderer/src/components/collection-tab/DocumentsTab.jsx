        ...
        <div className="modal-backdrop" onClick={onClose}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
                ...
                <Select
                    value={type}
                    onChange={setType}
                    options={FIELD_TYPES.filter((ft) => ft !== 'Object' && ft !== 'Array' && ft !== 'DBRef' && ft !== 'Binary').map((ft) => ({
                        value: ft,
                        label: ft
                    }))}
                />
                ...
                <div className="modal-actions">
                    ...
                    <button className="primary" onClick={handleApply}
                            disabled={busy}>{busy ? t('documentsTab.applying') : t('documentsTab.apply')}</button>
                </div>
            </div>
        </div>,
        document.body
...
export default function DocumentsTab({selection, reloadSignal, filterRequest}) {
    ...
    useEffect(() => {
        if (reloadSignal !== undefined) runQuery();
    }, [reloadSignal]);
    useEffect(() => {
    ...
    async function handleDeleteSelected() {
        ...
        const ok = await confirmDialog(t('documentsTab.confirmDeleteSelected', {count: selectedIds.size}), {
            title: t('documentsTab.deleteDocumentsTitle'),
            confirmLabel: t('documentActions.delete')
        });
        if (!ok) return;
    ...
    async function handleDeleteDoc(doc) {
        const ok = await confirmDialog(t('documentsTab.confirmDeleteOne'), {
            title: t('documentsTab.deleteDocumentTitle'),
            confirmLabel: t('documentActions.delete')
        });
        if (!ok) return;
        try {
        ...
        setRowContextMenu({
            ...
            items: [
                ...
                },
                {
                    ...
                    disabled: !hasValue,
                    onClick: () => copyToClipboard(toShellText(doc[field]))
                },
                {separator: true},
                {
                    label: t('documentActions.copyDocumentRaw'),
                ...
                {separator: true},
                {
                    label: t('documentActions.deleteField'),
                    danger: true,
                    disabled: !hasValue,
                    onClick: () => handleDeleteFieldValue(doc, field)
                }
            ]
        });
    }
    ...
    async function handleDeleteFieldValue(doc, field) {
        const ok = await confirmDialog(t('documentsTab.confirmDeleteField', {field}), {
            title: t('documentsTab.deleteFieldTitle'),
            confirmLabel: t('documentActions.delete')
        });
        if (!ok) return;
        try {
    ...
    async function handleDeleteFieldOnAll(field) {
        const ok = await confirmDialog(t('documentsTab.confirmDeleteFieldOnAll', {field}), {
            title: t('documentsTab.deleteFieldOnAllTitle'),
            confirmLabel: t('documentActions.delete')
        });
        if (!ok) return;
        ...
        <div className="documents-tab">
            <div className="query-bar">
                ...
                <div className="query-field">
                    ...
                    <input value={sort} onChange={(e) => setSort(e.target.value)}
                           placeholder={t('documentsTab.sortPlaceholder')}/>
                </div>
                <button className="primary" onClick={handleRunClick}
                        disabled={loading}>{loading ? t('documentsTab.running') : t('documentsTab.run')}</button>
            </div>

            <div className="toolbar">
                <button onClick={() => setModalDoc('new')}>{t('documentsTab.newDocument')}</button>
                <button onClick={handleDeleteSelected}
                        disabled={selectedIds.size === 0}>{t('documentsTab.deleteSelected', {count: selectedIds.size})}</button>
                <div className="spacer"/>
            ...
            <div className="results-area">
                <table className="doc-table spreadsheet-table">
                    ...
                    <tr>
                        ...
                        <th className="col-id sortable" onClick={() => handleSortClick('_id')}
                            title={t('documentsTab.sortByField', {field: '_id'})}>
                            {(() => {
                                const info = sortInfoFor('_id');
                                return (
                                    <span className={`sort-header ${info ? 'is-active' : ''}`}>
                        _id
                                        {info && currentSortEntries.length > 1 &&
                                            <span className="sort-priority">{info.priority}</span>}
                            ...
                            {fieldColumns.map((field) => {
                                ...
                                return (
                                    <td key={field} className="spreadsheet-cell"
                                        onContextMenu={(e) => handleCellContextMenu(e, doc, field, rowIndex)}>
                                        {isEditing ? (
                                            <InlineCellEditor
                                                value={doc[field]}
                    ...
                    {docs.length === 0 && !loading && (
                        <tr>
                            <td colSpan={fieldColumns.length + 3}
                                className="tree-empty">{t('documentsTab.noDocumentsFound')}</td>
                        </tr>
                    )}
                    </tbody>
                </table>
            ...
            <div className="pagination">
                ...
                <button disabled={page >= totalPages - 1}
                        onClick={() => setPage((p) => p + 1)}>{t('documentsTab.next')}</button>
            </div>
