...
function ValueEditor({value, onCommit, onCancel}) {
    ...
    return (
      ...
      <Select
          ...
          value={type}
          onChange={setType}
          options={FIELD_TYPES.filter((t) => t !== 'Object' && t !== 'Array' && t !== 'DBRef' && t !== 'Binary').map((t) => ({
              value: t,
              label: t
          }))}
      />
...
function NewFieldRow({isArray, onAdd, onCancel}) {
    ...
    return (
        <div className="tree-row new-field-row">
            {!isArray && (
                <input className="inline-input key-input" autoFocus placeholder={t('documentTree.fieldNamePlaceholder')}
                       value={key}
        ...
        return (
            <div className="tree-node-block">
                <div className="tree-row expandable-row"
                    ...
                    <i className={`fa-solid fa-chevron-right twisty ${!collapsed ? 'is-expanded' : ''}`}
                    ...
                    <div className="tree-children-block">
                        ...
                        {addingField ? (
                            ...
                            <div className="tree-row add-field-row" onClick={() => setAddingField(true)}>
                                ...
                                <span
                                    className="add-field-label">{Array.isArray(value) ? t('documentTree.addItem') : t('documentTree.addField')}</span>
                            </div>
                        )}
                    </div>
                )}
    ...
    return (
        <div className="tree-row leaf-row"
          ...
          >
            ...
            {onDelete && (
                <button className="tree-delete-btn" onClick={() => onDelete()}
                        title={t('documentTree.delete')}>✕</button>
            )}
        </div>
    );
