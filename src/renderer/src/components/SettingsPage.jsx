...
export default function SettingsPage({ onClose, connections, onImported }) {
  ...
  async function handleSetup(e) {
    ...
    if (newPw.length < 6) { setError('Use at least 6 characters.'); return; }
    setBusy(true);
      ...
      <div className="settings-page">
        ...
        <div className="settings-section">
          ...
          {!vaultStatus.encryptionEnabled && mode !== 'setup' && (
            <button className="primary" onClick={() => { resetForm(); setMode('setup'); }}>Set up master password</button>
          )}

          {vaultStatus.encryptionEnabled && !mode && (
            <div className="toolbar">
              <button onClick={() => { resetForm(); setMode('change'); }}>Change password</button>
              <button onClick={() => { resetForm(); setMode('disable'); }}>Remove master password</button>
            </div>
          )}

          {mode === 'setup' && (
            <form onSubmit={handleSetup}>
              <input type="password" placeholder="New master password" value={newPw} onChange={(e) => setNewPw(e.target.value)} autoFocus />
              ...
              <div className="modal-actions">
                ...
                <button type="submit" className="primary" disabled={busy}>{busy ? 'Setting up...' : 'Enable'}</button>
              </div>
            </form>
          )}
          ...
          {mode === 'change' && (
            <form onSubmit={handleChange}>
              <input type="password" placeholder="Current master password" value={currentPw} onChange={(e) => setCurrentPw(e.target.value)} autoFocus />
              <input type="password" placeholder="New master password" value={newPw} onChange={(e) => setNewPw(e.target.value)} />
              <input type="password" placeholder="Confirm new password" value={confirmPw} onChange={(e) => setConfirmPw(e.target.value)} />
              <div className="modal-actions">
                <div className="spacer" />
                <button type="button" onClick={resetForm}>Cancel</button>
                <button type="submit" className="primary" disabled={busy}>{busy ? 'Changing...' : 'Change'}</button>
              </div>
            </form>
          )}
          ...
          {mode === 'disable' && (
            <form onSubmit={handleDisable}>
              ...
              <div className="modal-actions">
                <div className="spacer" />
                <button type="button" onClick={resetForm}>Cancel</button>
                <button type="submit" className="danger" disabled={busy}>{busy ? 'Removing...' : 'Remove'}</button>
              </div>
            </form>
          )}
        </div>
        ...
        <div className="settings-section">
          <h3>Backup</h3>
          <p className="settings-section-desc">Export or import all settings and saved connections as a single JSON file.</p>
          <div className="toolbar">
